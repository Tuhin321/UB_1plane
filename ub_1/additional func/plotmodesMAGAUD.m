function plotmodesMAGAUD(Inp,filename)
...
T = readtable([filename,'_modes.dat']);
T( : , end)=[];
Tmodesdouble=table2array(T);
figure
plot(Tmodesdouble(:,4:7))
return

fid=fileread([filename,'.out']);
node_spectrum = textscan(fid,'%f%f%f%f%f%f','Delimiter',',','HeaderLines',17);
fid = fclose('all');

inch2meter=2.54e-2;
nodeloc=node_spectrum{end}.*inch2meter; % position

% figure
        for i =1:4
                % Scaling mode shapes
        maxValue = max(abs(Tmodesdouble(:,i+3)));
        maxWantedValue = 0.4;
        ...
        %maxWantedValue = 1;
        hold on
        h(i)=plot(nodeloc,Tmodesdouble(:,i+3)/maxValue*maxWantedValue,'-','linewidth',2) ;
 
%             plot( nodeloc,Tmodesdouble(:,i),'-','linewidth',2)
%             ylim (hay,[0, 5e-5])
%             t = annotation('textbox');
%             sz = t.FontSize;
%             t.FontSize = 12;
%              text(15 , 0.03 ,{num2str(FreqlistReal(i),3) 'Hz'},'Color','black','FontSize',12)
            xlabel('Rotor length (m)')
            ylabel('Deformation') 
%             ticks=nodeloc
            str{i}=['mode ',num2str(i+3)];
...
%             set(gcf,'renderer','Painter', 'Position', [100, 100, 560, 420/2])
        end
        grid minor
        legend(h,str);
        title('Free Free flexible modes from MAGAUD')
hold off
end
