function f_MaximumCapacity_v3_3AMB(Fsat,wrangef,filename,gap,sys)
...
A=A_rotor;
outscale=diag([1/gap1 1/gap2 1/gap3 1/gap4 1/Fsat1 1/Fsat2 1/Fsat3]); %normalize outputs to maximum values

Bu=B_rotor(:,2:end); %w(impeller)
Bw=B_rotor(:,1); %u1 and u2 (amb)

C_rotor=[C_rotor(1:end,:); zeros(1,size(A_rotor,1)); zeros(1,size(A_rotor,1)); zeros(1,size(A_rotor,1))]; %position (m) @ (journal %force (N) )  
C=outscale*C_rotor;
%            u1 u2 u3
Du=outscale*[0  0  0  ;   %x1
             0  0  0  ;   %x2
             0  0  0  ;   %x3
             0  0  0  ;   %x4
             1  0  0  ;   %Famb1
...
Dw=outscale*[0 0 0 0 0 0 0]'; %w

I=eye(max(size(A)));
II=eye(7);
fm=wrangef;
rm=[fm;fm;fm;fm;fm;fm;fm];
